...
REPO="${WAVER_REPO:-$HOME/ugv_ws/FSD_Vehicle}"
WS="${WAVER_WS:-$REPO}"
cd "$REPO"
...
count_publishers() {
  local topic="$1"
  ros2 topic info -v "$topic" 2>/dev/null | grep -c 'Endpoint type: PUBLISHER' || true
}
...
if printf '%s\n' "$CMD_INFO" | grep -E 'controller_server|waver_remote_panel|teleop|ugv_driver|target_goal|pointcloud' >/dev/null; then
...
if printf '%s\n' "$SAFETY" | grep -E 'STOP|EMERGENCY|FAULT' >/dev/null; then
  warn_or_fail "safety_state indicates stop/fault"
fi
