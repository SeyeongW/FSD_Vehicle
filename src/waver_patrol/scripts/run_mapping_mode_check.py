...
class MappingModeCheck(Node):
    ...
    def maybe_send_commands(self) -> None:
        ...
        if (not self.save_sent) and elapsed >= self.args.save_after_sec:
            ...
            self.save_sent = True
        if self.save_sent and (not self.apply_sent) and elapsed >= self.args.save_after_sec + self.args.apply_after_save_sec:
            self.command_pub.publish(String(data="START_LOCALIZATION"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--apply-after-save-sec", type=float, default=2.0)
