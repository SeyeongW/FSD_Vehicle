...
used for track continuity and ID-switch metrics. Gazebo smoke results are
simulation-based pipeline evidence only and must be tied to current-schema logs
before citation.
...
- Gazebo/UI mapping validation is simulation-based evidence. It supports pre-deployment readiness only when current-schema logs show PASS; wheel-on PASS still requires real_vehicle_precheck, rosbag replay, wheel-off HIL, hardware E-STOP, and supervised low-speed closed-area trials.
