...
```bash
WORKSPACE_ROOT=$HOME/ros2_ws2/FSD_Vehicle
cd $WORKSPACE_ROOT
...
ros2 launch waver_patrol waver_gazebo_mapping_debug.launch.py \
  ...
  save_dir:=$HOME/ros2_ws2/FSD_Vehicle/maps
```
...
python3 src/waver_patrol/scripts/waver_map_quality_check.py \
  ~/ros2_ws2/FSD_Vehicle/maps/waver_latest_map.yaml
```
...
under `~/ros2_ws2/FSD_Vehicle/experiments_result`.
...
ros2 launch waver_patrol waver_real_bird_autonomy.launch.py \
  ...
  odom_source:=ekf \
  map:=$HOME/ros2_ws2/FSD_Vehicle/maps/waver_latest_map.yaml \
