...
# .env 파일에서 환경변수 로드 (ROS2_WS5_PATH 제외)
if [ -f "${SCRIPT_DIR}/.env" ]; then
    while IFS='=' read -r key value; do
        # 주석과 빈 줄 무시, ROS2_WS5_PATH는 아래에서 자동 설정
        [[ "$key" =~ ^#.*$ || -z "$key" || "$key" == "ROS2_WS5_PATH" ]] && continue
        # 값에서 앞뒤 공백 제거
...
# ROS2_WS5_PATH를 항상 현재 스크립트 위치 기준으로 자동 설정
export ROS2_WS5_PATH="${SCRIPT_DIR}"

echo "============================================="
echo " FSD_Vehicle 로컬 환경 설정 완료"
echo " ROS2_WS5_PATH = ${ROS2_WS5_PATH}"
